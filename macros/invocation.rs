...
impl Parse for TemplateInvocation {
    fn parse(input: ParseStream) -> syn::Result<Self> {
        ...
        Ok(TemplateInvocation { driver, colon, template })
    }
}
...
pub fn derive_adhoc_func_macro(input: TokenStream)
    ...
    let TemplateInvocation { driver, colon, template } = syn::parse2(input)?;
    ...
    let mac_name = {
        let mut name = driver;
        let last = name.segments.last_mut().ok_or_else(|| syn::Error::new(
            colon.span,
